...
# color palette ---
cmv_cols <- c('1'='#faa31b', #pos
              '0'='#1eb8d4') #neg 
p_cmv <- pheno %>%
  group_by(Age_groups, infered_CMV) %>%
  summarise(n = n(), .groups = "drop_last") %>%
  mutate(freq = n / sum(n) * 100) %>%
  mutate(Age_groups = factor(Age_groups, levels = age_groups)) %>%
  ggplot(aes(x = "", y = freq, fill = as.character(infered_CMV))) +
...
ggsave("./Piechart_CMV_ageGroups_03182026.pdf", p_cmv,
