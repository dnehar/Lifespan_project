...
p_tonic <- LifeSpan_ALL_MetaData %>% 
     ...
     ungroup() %>%
     as.data.frame() %>%
     filter(ReCluster %in% c('CD14_mo_ISGhi')) %>% 
     
     #plot the number of cells
     ggplot(aes(x=Names, y=n)) + 
     geom_bar(stat="identity",fill="#f15d64", color="black") + #CD4 T cells:  #697d35 #CD14 mo: #f15d64,  B cell: 
