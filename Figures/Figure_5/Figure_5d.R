...
box_plot_pbmc_L2 <- LifeSpan_ALL_MetaData %>%
  ...
  ungroup() %>%
  as.data.frame() %>%
  filter(ReCluster %in% subset_to_be_plotted) %>%                 # keep Treg subtypes only
  ...
  facet_wrap(. ~ ReCluster, scales = "free_y", nrow = 1) +       # one panel per Treg subtype

  scale_fill_manual(values = cols) +                              # apply Treg subtype color palette
