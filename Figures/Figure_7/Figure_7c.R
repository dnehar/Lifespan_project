...
# Step 4: keep only the two SOX4 CD4 T cell subtypes of interest
...
box_plot_pbmc_L2 <- LifeSpan_ALL_MetaData %>%
  ...
  filter(ReCluster %in% subset_to_be_plotted) %>%                 # keep SOX4 CD4 T cell subtypes only
  ...
  facet_wrap(. ~ ReCluster, scales = "free_y", nrow = 1) +       # one panel per SOX4 CD4 T cell subtype

  scale_fill_manual(values = cols) +                              # apply SOX4 CD4 T cell subtype color palette
...
# --- Save figure as PDF ---
ggsave("./boxplot_SOX4_CD4_T_cells_in_PBMCs_03132026.pdf", box_plot_pbmc_L2,
