# =============================================================================
# Figure 2g — Stacked bar plot of monocyte subset proportions across age groups (Level 4)
#
# This script computes frequencies of three monocyte subtypes
# (CD14_mono, ISGhi_CD14_mono, CD16_mono) as a percentage of all monocyte cells,
# and displays their composition across seven age groups as a stacked bar plot.
# Input:  pbmcs_v1.rds  — available at dnehar/Lifespan_project/pbmcs_v1.rds
# Output: ./Barplot_monocytes_age_groups_01062026.pdf
# =============================================================================

library(dplyr); library(ggplot2)

# --- Color palette — one color per monocyte subtype (Level 4 annotation) ---
cols <- c(
  "CD14_mono"       = "#f6a2a7",
  "CD16_mono"       = "#f9d3d7",
...
# --- Define the monocyte subtypes to plot (Level 4 annotation) ---
...
# --- Compute monocyte subtype proportions and build stacked bar plot ---
# Step 1: assign ordered factor levels to age group (Groups) and monocyte subtype (ReCluster)
# Step 2: filter to keep only the three monocyte subtypes of interest
# Step 3: count cells per age group x monocyte subtype combination
# Step 4: compute frequency as % of total monocyte cells within each age group
# Step 5: plot stacked bar chart — one bar per age group, filled by monocyte subtype
BP_1 <- LifeSpan_ALL_MetaData %>%

  mutate(Groups    = factor(Age_groups, levels = age_groups)) %>%        # ordered age groups
  mutate(ReCluster = factor(LS_L4, levels = subset_to_be_plotted)) %>%   # Level 4 monocyte annotation
  ...
  mutate(freq = n / sum(n) * 100) %>%                                    # % of monocyte cells per age group
  ungroup() %>%
  as.data.frame() %>%
  ...
  scale_fill_manual(values = cols) +                                     # apply monocyte color palette
...
# --- Save output ---
ggsave("./Barplot_monocytes_age_groups_01062026.pdf", BP_1,
